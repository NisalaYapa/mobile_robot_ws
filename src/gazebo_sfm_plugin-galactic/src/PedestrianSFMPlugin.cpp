...
void PedestrianSFMPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("should_rotate"))
    this->should_rotate = 1 ;
  else
  ...
  this->positionPub = this->node->Advertise<msgs::Pose>(this->topic_name);
...
void PedestrianSFMPlugin::OnUpdate(const common::UpdateInfo &_info) {
  ...
  if (this->should_rotate == 1) {                                                             // added by nisala
    ...
    actorPose.Rot() = ignition::math::Quaterniond(1.57, 0, yaw);
  ...
  // actorPose.Pos().Z(1.0138);                                // changed
